...
$pw = Read-Host -AsSecureString "Choose a password for the .pfx (you'll set this as WIN_CSC_KEY_PASSWORD)"
...
Write-Host "  bib-ordre-signing.cer  <- distribute this to shop PCs and import into 'Trusted Root Certification Authorities'"
Write-Host ""
Write-Host "Next:"
Write-Host "  `$env:WIN_CSC_LINK = `"$pfxPath`""
Write-Host "  `$env:WIN_CSC_KEY_PASSWORD = `"<the password you just chose>`""
